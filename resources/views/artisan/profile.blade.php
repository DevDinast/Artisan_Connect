...
<div class="container" style="max-width: 800px; margin: auto">
    ...
    <div class="dashboard-hero" style="margin-bottom: 2rem">
        <div class="dashboard-avatar" style="font-size: 1.5rem; width: 5rem; height: 5rem">
            @if($artisan->avatar)
        ...
        <a href="https://wa.me/{{ preg_replace('/\D/', '', $artisan->telephone) }}"
           target="_blank"
           class="btn"
           style="margin-left: auto; background: #25D366; border-color: #25D366">
            💬 WhatsApp
        </a>
        @endif
    </div>
...
async function loadOeuvres() {
    ...
    try {
        const res     = await fetch('/api/v1/catalog/artisans/{{ $artisan->id }}', {
        ...
        grid.innerHTML = oeuvres.map(o => {
            const image = o.image ?? 'https://via.placeholder.com/400x300?text=Oeuvre';
            const prix  = Number(o.prix).toLocaleString('fr-FR') + ' FCFA';
            return `
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <img src="${image}" alt="${o.titre}" class="w-full h-48 object-cover">
                <div class="p-4">
                    <h3 class="text-lg font-semibold mb-2">${o.titre}</h3>
                    <p class="text-gray-600 mb-2">${o.categorie ?? ''}</p>
                    <p class="text-gray-800 font-bold mb-4">${prix}</p>
